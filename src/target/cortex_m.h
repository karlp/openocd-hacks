...
/**
 * @returns cached value of Cortex-M cpuid
 * or CORTEX_M_PARTNO_INVALID if the magic number does not match
 * or core_info is not initialised.
 */
static inline enum cortex_m_cpuid cortex_m_get_cpuid_safe(struct target *target)
{
	...
	return cortex_m->core_info->cpuid;
}
